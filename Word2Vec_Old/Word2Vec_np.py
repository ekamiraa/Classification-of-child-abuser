...
class Word2VecModel:
    def __init__(self, filename, embedding_dim=100, window_size=2, learning_rate=0.1, epochs=2):
        ...
        self.word_to_id, self.id_to_word = self.create_vocabulary(self.flattened_tokens)
    ...
    def initialize_weights(self, vocab_size):
        ...
        w = np.random.rand(vocab_size, self.embedding_dim)  # Input weights
        v = np.random.rand(self.embedding_dim, vocab_size)  # Output weights
        return w, v
    ...
    def backward_propagation(self, word, context, A, Z, w, v):
        ...
        dv = np.outer(A, dB)
        dA = np.dot(dB, v.T)
        dw = np.outer(dA, self.one_hot_encode(word, len(w))).T
    ...
    def aggregate_vectors_per_document(self, data):
        ...
        for tokens in data:
            ...
            doc_vector = np.mean(self.w[token_indices], axis=0)
            V.append(doc_vector)
        return np.array(V)
...
if __name__ == "__main__":
    ...
    output_file = 'Word2vec_Vector_3_0.01_50.csv'
